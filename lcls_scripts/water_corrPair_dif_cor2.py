...
f = h5py.File('/reg/d/psdm/cxi/cxilp6715/scratch/combined_tables/finer_q/run47.tbl','r')
f_out = h5py.File('/reg/d/psdm/cxi/cxilp6715/scratch/corr_pair/run47_corrPair_corr_%d_max100.h5'%qidx4pairing,'w')
#######################
use_basic_mask= True
...
for cluster_num in cluster_to_use:
    ...
    if max_cluster_size is None or num_shots>max_cluster_size:

        cluster_max_pos = max_pos[pulse_energy_clusters==cluster_num]
        if max_cluster_size is None:
            bins = np.histogram(cluster_max_pos,bins='fd')
        else:
            n_bins = (int(num_shots/max_cluster_size) + 1)*2
            bins = np.histogram(cluster_max_pos,bins=n_bins)
        ...
        shots = PI[sorted(shot_tags)]
        print "number of shots in pairing cluster: %d"% len(shot_tags)
        # mask and normalize the shots
        if shots.dtype != 'float64':
            # shots need to be float64 or more. 
            # float32 resulted in quite a bit of numerical error 
