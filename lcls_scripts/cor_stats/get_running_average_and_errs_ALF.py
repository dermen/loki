...
output_fname='/reg/d/psdm/cxi/cxilp6715/results/running_ave_cors/ALF_pros_allshots.h5'
phi_offset = 30
num_phi = 354


run_nums=[51,51,51,51,50,50,50]
with h5py.File(output_fname,'w') as f_out:
    ...
    for r_num in run_nums:
        ...
        if r_num==51:
            counter+=1
        elif r_num == 50 and counter==4:
            counter =1
        else:
            counter+=1

        grp = f_out.create_group('run%d_%d'%(r_num, counter) )
        ...
        for qidx in range(35):
            ...
            GDPpro = get_shots(r_num,'ALF_pro',qidx, num_shots=36000*counter)[:,phi_offset:num_phi/2-phi_offset]
...
output_fname='/reg/d/psdm/cxi/cxilp6715/results/running_ave_cors/ALF_bufs_allshots.h5'
phi_offset = 30
num_phi = 354


run_nums=[49,49,49,49,48,48,48]
with h5py.File(output_fname,'w') as f_out:
    f_out.create_dataset('phi_offset',data=phi_offset)
    counter=0
    for r_num in run_nums:
        ...
        for qidx in range(35):
            ...
            GDPpro = get_shots(r_num,'ALF_buf',qidx, num_shots=36000*counter)[:,phi_offset:num_phi/2-phi_offset]
            norm_GDPpro = normalize_set(GDPpro)
