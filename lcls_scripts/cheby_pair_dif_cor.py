...
for ll in unique_labels:
    ...
    print("number of shots in this cluster: %d"%(shots.shape[0]))
    ...
    # merge shots
    shots_merge = np.zeros( (shots.shape[0], 3, shots.shape[-1])
        ,dtype=np.float64 )
    for idx,ss in enumerate(shots):
        shots_merge[idx,0] = ss[:10].sum(0)
        shots_merge[idx,1] = ss[10:20].sum(0)
        shots_merge[idx,2] = ss[20:].sum(0)
