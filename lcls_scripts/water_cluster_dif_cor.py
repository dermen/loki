...
f_out = h5py.File('/reg/d/psdm/cxi/cxilp6715/scratch/water_data/run94_dif_int_basic_pmask_removePeak.h5','w')
#######################
use_basic_mask= True
if 'polar_mask_binned' in f.keys():
    pmask_basic = np.array(f['polar_mask_binned'].value==f['polar_mask_binned'].value.max(), dtype = int)
else:
