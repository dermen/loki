...
for qidx in qcluster_inds:
    ...
    norm_shots=norm_shots/norm_shots.max(-1)[:,:,None]
    ...
    # now make streak masks for each cluster and save them
    print('making streak masks...')
    all_streak_masks=[]
    # for ll in [4,5,9]:
    for ll in np.unique(kmeans.labels_):
        #maks the streak_mask
        ss=shots_to_use[kmeans.labels_==ll]
        mean_ss=ss.mean(0)
        streak_mask = ~(mean_ss[0]>(mean_ss[0].std()/10.))
        all_streak_masks.append(streak_mask)
    all_streak_masks=np.array(all_streak_masks).astype(bool)
    f_out.create_dataset('q%d/streak_masks'%qidx,data=all_streak_masks)
